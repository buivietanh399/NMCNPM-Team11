...
public class HoKhauRepositoryImpl implements  HoKhauRepository {
    ...
    //cho thống kê
    @Override
    public int tongHoKhauThuongTru(int year){
        int tongHoKhauThuongTru = 0;

        try {
            conn = DbUtil.getInstance().getConnection();
            pstmt = conn.prepareStatement(SQLCommand.HO_KHAU_QUERY_TONG_THUONG_TRU_NAM);
            rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    @Override
    public int tongHoKhauDaChuyenDi(int year){
        int tongHoKhauDaChuyenDi = 0;

        try {
            conn = DbUtil.getInstance().getConnection();
            pstmt = conn.prepareStatement(SQLCommand.HO_KHAU_QUERY_TONG_DA_CHUYEN_DI_NAM);
            rs = pstmt.executeQuery();
            while (rs.next()) {
                if( Integer.parseInt(rs.getString(1)) <= year)
                tongHoKhauDaChuyenDi ++;
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    //cho thong ke
    @Override
    public HashMap<Integer, Integer> soLuongThanhVien(int year) {
        HashMap<Integer, Integer> soLuongThanhVien = new HashMap<>();

        try {
            conn = DbUtil.getInstance().getConnection();
            pstmt = conn.prepareStatement(SQLCommand.HO_KHAU_QUERY_SO_LUONG_THANH_VIEN_NAM);
            rs = pstmt.executeQuery();
            while (rs.next()) {
                if(Integer.valueOf(rs.getInt(1)) <= year)
                soLuongThanhVien.put(Integer.valueOf(rs.getInt(2)), Integer.valueOf(rs.getInt(3)));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
    ...
    public void delete_chuho_cu( int idChuHo){
        try {
            conn = DbUtil.getInstance().getConnection();
            pstmt = conn.prepareStatement(SQLCommand.HO_KHAU_DELETE_XOA_CHU_HO_CU);
